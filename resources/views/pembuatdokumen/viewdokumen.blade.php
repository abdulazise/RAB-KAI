...
<html lang="en">
  <head>
    ...
    <title>Daftar Pembuat Dokumen</title>
...
<div class="pb-3">
    <form action="{{ url('viewdokumen') }}" method="get">
        ...
        <!-- TOMBOL TAMBAH DATA -->
        <div class="pb-3">
          <a href="{{ route('tambahdokumen') }}" class="btn btn-primary">+ Tambah Data</a>
        </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $item->jabatan }}</td>
              <td>
                <a href="{{ url('tambahdokumen/' . $item->NIPP . '/editdokumen') }}" class="btn btn-warning btn-sm">Edit</a>
                <form action="{{ url('tambahdokumen/' . $item->NIPP) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
