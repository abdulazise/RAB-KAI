...
<html lang="en">
  <head>
    ...
    <title>Daftar Pejabat Menyetujui</title>
...
<div class="pb-3">
    <form action="{{ url('viewmengetahui') }}" method="get">
        ...
        <!-- TOMBOL TAMBAH DATA -->
        <div class="pb-3">
          <a href="{{ route('tambahmenyetujui') }}" class="btn btn-primary">+ Tambah Data</a>
        </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $item->jabatan}}</td>
              <td>
                <a href="{{ url('tambahmenyetujui/' . $item->NIPP . '/editmenyetujui') }}" class="btn btn-warning btn-sm">Edit</a>
                <form action="{{ url('tambahmenyetujui/' . $item->NIPP) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
