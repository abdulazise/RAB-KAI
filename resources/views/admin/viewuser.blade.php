...
<div class="pb-3">
    <form action="{{ url('viewuser') }}" method="get">
        ...
        <!-- TOMBOL TAMBAH DATA -->
        <div class="pb-3">
          <a href="{{ route('adduser') }}" class="btn btn-primary">+ Tambah Data</a>
        </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $item->role }}</td>
              <td>
                <a href="{{ url('adduser/' . $item->NIPP . '/edit') }}" class="btn btn-warning btn-sm">Edit</a>
                <form action="{{ url('adduser/' . $item->NIPP) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
