...
<div class="pb-3">
    <form action="{{ url('viewdaop') }}" method="get">
        ...
        <!-- TOMBOL TAMBAH DATA -->
        <div class="pb-3">
          <a href="{{ route('pengaturandaop') }}" class="btn btn-primary">+ Tambah Data</a>
        </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $item->nama }}</td>
              <td>
                <a href="{{ url('pengaturandaop/' . $item->nama . '/edit') }}" class="btn btn-warning btn-sm">Edit</a>
                <form action="{{ url('pengaturandaop/' . $item->nama) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
