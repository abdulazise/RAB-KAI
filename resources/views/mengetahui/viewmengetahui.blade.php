...
<html lang="en">
  <head>
    ...
    <title>Daftar Pejabat Mengetahui</title>
...
<div class="pb-3">
    <a href="{{ url('adduser') }}" class="btn btn-secondary" style="background-color: orange; border-color: orange; color: white;">Kembali</a>
</div>
...
<div class="pb-3">
    <form action="{{ url('viewmengetahui') }}" method="get">
        ...
        <!-- TOMBOL TAMBAH DATA -->
        <div class="pb-3">
          <a href="{{ route('tambahmengetahui') }}" class="btn btn-primary">+ Tambah Data</a>
        </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $item->jabatan}}</td>
              <td>
                <a href="{{ url('tambahmengetahui/' . $item->NIPP . '/editmengetahui') }}" class="btn btn-warning btn-sm">Edit</a>
                <form action="{{ url('tambahmengetahui/' . $item->NIPP) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
